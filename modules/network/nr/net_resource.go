...
import (
	...
	"github.com/threefoldtech/testv2/modules/network/nft"
	"github.com/threefoldtech/testv2/modules/network/wireguard"
...
func New(nodeID string, network *modules.Network, privateKey wgtypes.Key) (*NetResource, error) {
	...
	nr.nibble, err = testip.NewNibble(nr.resource.Prefix, nr.allocNr)
	if err != nil {
		return nil, err
	}
...
func (nr *NetResource) Nibble() *testip.Nibble {
	return nr.nibble
}
...
func (nr *NetResource) routes() ([]*netlink.Route, error) {
	...
	for _, peer := range nr.resource.Peers {
		...
		nibble, err := testip.NewNibble(peer.Prefix, nr.allocNr)
		if err != nil {
			return nil, err
		}
	...
	for _, peer := range nr.resource.Peers {
		...
		if prefixStr != nr.exit.Prefix.String() {
			...
			if isIn(peer.Prefix.String(), nr.publicPrefixes) {
				wgPeer.Endpoint = testip.WGEndpoint(peer)
			}
			...
			allowedIPs := testip.WGExitPeerAllowIPs()
			wgPeers = append(wgPeers, &wireguard.Peer{
				PublicKey: exitPeer.Connection.Key,
				Endpoint:  testip.WGEndpoint(exitPeer),
				AllowedIPs: []string{
					allowedIPs[0].String(),
					allowedIPs[1].String(),
				},
...
func (nr *NetResource) GWTNRoutesIPv4() ([]*netlink.Route, error) {
	...
	for _, peer := range nr.resource.Peers {
		peerNibble, err := testip.NewNibble(peer.Prefix, nr.allocNr)
		if err != nil {
			return nil, err
		}
