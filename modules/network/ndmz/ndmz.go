...
import (
	...
	"github.com/threefoldtech/testv2/modules/network/nr"

	"github.com/threefoldtech/testv2/modules/network/dhcp"

	"github.com/threefoldtech/testv2/modules/network/macvlan"
...
func attachVeth(netNS ns.NetNS) error {
	...
	if !ifaceutil.Exists(vethHost, nil) || !ifaceutil.Exists(vethNDMZ, netNS) {
		if _, _, err := ip.SetupVethWithName(vethHost, vethNDMZ, 1500, netNS); err != nil {
			return errors.Wrap(err, "failed to create veth pair to connect test bridge and ndmz bridge")
		}
	}
	log.Info().
		Str("ndmz side", vethNDMZ).
		Str("host side", vethHost).
		Msg("veth pair for ndmz to public test bridge created")
	...
	br, err := bridge.Get("test") //TODO: use costant
	if err != nil {
		return err
	}
