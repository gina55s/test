...
func EnsureWakeOnLan(ctx context.Context) (bool, error) {
	...
	if err != nil {
		return false, errors.Wrap(err, "failed to get test bridge")
	}

	nics, err := bridge.ListNics(inf, true)
	if err != nil {
		return false, errors.Wrap(err, "failed to list attached nics to test bridge")
	}
	...
	if len(filtered) != 1 {
		return false, fmt.Errorf("test bridge has multiple interfaces")
	}
...
func (p *PowerServer) powerUp(node *substrate.Node, reason string) error {
	...
	for i := 0; i < 10; i++ {
		if err := exec.Command("ether-wake", "-i", "test", mac).Run(); err != nil {
