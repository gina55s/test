...
// fresh disks to use from test v1, in case you moving
...
loop:
	for _, device := range devices {
		if strings.HasPrefix(device.Label, testV1PoolPrefix) {
			destroyMe = append(destroyMe, device)
			continue
		}

		for _, child := range device.Children {
			if strings.HasPrefix(child.Label, testV1PoolPrefix) {
				destroyMe = append(destroyMe, device)
				continue loop
			}
		}
