...
func (s *Module) initialize(policy pkg.StoragePolicy) error {
	...
	for idx := range freeDisks {
		if freeDisks[idx].DiskType == test.SSDDevice {
...
func (s *Module) VDiskFindCandidate(size uint64) (path string, err error) {
	candidates, err := s.findCandidates(size, test.SSDDevice)
	if err != nil {
		return path, err
	}
...
func (s *Module) ensureCache() error {
	...
	if cacheFs == nil {
		...
		fs, err := s.createSubvolWithQuota(cacheSize, cacheLabel, test.SSDDevice)
	...
	if cacheFs == nil {
		...
		fs, err := s.createSubvolWithQuota(cacheSize, cacheLabel, test.HDDDevice)
...
func (s *Module) createSubvol(size uint64, name string, poolType pkg.DeviceType) (filesystem.Volume, error) {
	...
	if poolType != test.HDDDevice && poolType != test.SSDDevice {
