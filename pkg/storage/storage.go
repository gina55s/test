...
func (s *Module) dump() {
	...
	for _, pool := range s.ssds {
		...
		device := pool.Device()
		log.Debug().Str("path", device.Path).Str("label", pool.Name()).Str("type", string(test.SSDDevice)).Send()
	}
	...
	for _, pool := range s.hdds {
		...
		device := pool.Device()
		log.Debug().Str("path", device.Path).Str("label", pool.Name()).Str("type", string(test.HDDDevice)).Send()
	}

}

// poolType gets the device type of a disk
func (s *Module) poolType(pool filesystem.Pool, vm bool) (test.DeviceType, error) {
	var typ test.DeviceType
	device := pool.Device()
	...
	for _, hdd := range s.hdds {
		...
		for _, vol := range volumes {
			if vol.Name() != zdbVolume && vol.Name() != cacheLabel {
				// we don't delete zdb volumes, also test-cache volumes
...
func (s *Module) findCandidates(size gridtypes.Unit) ([]candidate, error) {
	...
	if len(candidates) == 0 {
		return nil, pkg.ErrNotEnoughSpace{DeviceType: test.SSDDevice}
	}
