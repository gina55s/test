...
type Container = test.Container

// ContainerResult type alias
type ContainerResult = test.ContainerResult
...
func (p *Primitives) containerProvisionImpl(ctx context.Context, wl *gridtypes.WorkloadWithID) (ContainerResult, error) {
	...
	netID := test.NetworkID(deployment.TwinID, config.Network.Network)
	log.Debug().
	...
	for _, mount := range config.Mounts {
		volume, err := deployment.GetType(mount.Volume, test.VolumeType)
		if err != nil {
			return ContainerResult{}, err
		}
...
func (p *Primitives) containerDecommission(ctx context.Context, wl *gridtypes.WorkloadWithID) error {
	...
	netID := test.NetworkID(deployment.TwinID, string(config.Network.Network))
