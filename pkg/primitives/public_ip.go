...
func (p *Primitives) getAssignedPublicIP(ctx context.Context, wl *gridtypes.WorkloadWithID) (result test.PublicIPResult, err error) {
	...
	ipWorkloads := deployment.ByType(test.PublicIPType)
	...
	for _, ip := range reserved {
		if _, ok := usedIPs[ip.IP]; !ok {
			...
			return test.PublicIPResult{IP: ipNet, Gateway: gw}, nil
		}
	}
...
func (p *Primitives) publicIPProvisionImpl(ctx context.Context, wl *gridtypes.WorkloadWithID) (result test.PublicIPResult, err error) {
	...
	if err != nil {
		return test.PublicIPResult{}, errors.Wrap(err, "could not look up ipv6 prefix")
	}
