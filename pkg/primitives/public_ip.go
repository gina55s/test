...
func (p *Primitives) getPublicIPData(ctx context.Context, wl *gridtypes.WorkloadWithID) (result test.PublicIP, err error) {
	switch wl.Type {
...
func (p *Primitives) publicIPProvisionImpl(ctx context.Context, wl *gridtypes.WorkloadWithID) (result test.PublicIPResult, err error) {
	config, err := p.getPublicIPData(ctx, wl)
	if err != nil {
