...
func (p *Primitives) publicIPProvisionImpl(ctx context.Context, wl *gridtypes.Workload) (result test.PublicIPResult, err error) {
	config := test.PublicIP{}

	network := stubs.NewNetworkerStub(p.zbus)

	if err := json.Unmarshal(wl.Data, &config); err != nil {
		return test.PublicIPResult{}, errors.Wrap(err, "failed to decode reservation schema")
	}

	if err := config.Valid(); err != nil {
		return test.PublicIPResult{}, errors.Wrap(err, "failed to validate ip reservation")
	}
