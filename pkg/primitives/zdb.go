...
import (
	...
	"github.com/threefoldtech/test/pkg/zdb"

	"github.com/pkg/errors"
	"github.com/rs/zerolog/log"
	"github.com/threefoldtech/test/pkg"
	nwmod "github.com/threefoldtech/test/pkg/network"
...
const (
	...
	zdbFlistURL         = "https://hub.grid.tf/tf-autobuilder/threefoldtech-0-db-development-v2-6c844b9a1c.flist"
	zdbContainerNS      = "zdb"
...
func (p *Primitives) zdbProvisionImpl(ctx context.Context, wl *gridtypes.WorkloadWithID) (test.ZDBResult, error) {
	var (
		//contmod = stubs.NewContainerModuleStub(p.zbus)
	...
	if err := json.Unmarshal(wl.Data, &config); err != nil {
		return test.ZDBResult{}, errors.Wrap(err, "failed to decode reservation schema")
	}
	...
	if err != nil {
		return test.ZDBResult{}, errors.Wrap(err, "failed to list container data volumes")
	}
	...
	for id, container := range containers {
		...
		for _, ns := range nss {
			...
			if err != nil {
				return test.ZDBResult{}, errors.Wrap(err, "failed to find IP address on zdb0 interface")
			}

			return test.ZDBResult{
				Namespace: nsID,
		...
		if err != nil {
			return test.ZDBResult{}, errors.Wrap(err, "failed to check total reserved size")
		}
	...
	if len(candidates) > 0 {
		...
		if err != nil {
			return test.ZDBResult{}, errors.Wrap(err, "couldn't allocate device to satisfy namespace size")
		}
		cont, err = p.ensureZdbContainer(ctx, device)
		if err != nil {
			return test.ZDBResult{}, errors.Wrap(err, "failed to start zdb container")
		}
	}
	...
	if err := p.createZDBNamespace(pkg.ContainerID(cont.Name), nsID, config); err != nil {
		return test.ZDBResult{}, errors.Wrap(err, "failed to create zdb namespace")
	}
...
func (p *Primitives) zdbUpdateImpl(ctx context.Context, wl *gridtypes.WorkloadWithID) (result test.ZDBResult, err error) {
	...
	if new.Size < old.Size {
		...
		// While this makes sense fro ZDB. it will not make test able to calculate the namespace
