...
import (
	...
	"github.com/threefoldtech/test/pkg/provision/common"
...
func (p *Primitives) zdbProvisionImpl(ctx context.Context, wl *gridtypes.Workload) (test.ZDBResult, error) {
	var (
	...
	if err != nil {
		return test.ZDBResult{}, errors.Wrap(err, "failed to decrypt namespace password")
	}
	...
	if err != nil {
		return test.ZDBResult{}, errors.Wrap(err, "failed to allocate storage")
	}
	...
	if err != nil {
		return test.ZDBResult{}, errors.Wrapf(err, "failed to ensure zdb containe running")
	}
...
func (p *Primitives) zdbRootFS() (string, error) {
	...
	for _, typ := range []test.DeviceType{test.HDDDevice, test.SSDDevice} {
