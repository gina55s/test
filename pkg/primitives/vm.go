...
const (
	...
	cloudContainerFlist = "https://hub.grid.tf/omar0.3bot/cloud-container.flist"
	cloudContainerName  = "cloud-container"
)

// ZMachine type
type ZMachine = test.ZMachine

// ZMachineResult type
type ZMachineResult = test.ZMachineResult
...
func (p *Primitives) vmMounts(ctx context.Context, deployment gridtypes.Deployment, mounts []test.MachineMount, format bool, vm *pkg.VM) error {
	for _, mount := range mounts {
		...
		switch wl.Type {
		case test.ZMountType:
			if err := p.mountDisk(ctx, wl, mount, format, vm); err != nil {
				return err
			}
		case test.QuantumSafeFSType:
			if err := p.mountQsfs(wl, mount, vm); err != nil {
				return err
			}
		default:
			return fmt.Errorf("expecting a reservation of type '%s' or '%s' for disk '%s'", test.ZMountType, test.QuantumSafeFSType, mount.Name)
		}
	}
	return nil
}

func (p *Primitives) mountDisk(ctx context.Context, wl *gridtypes.WorkloadWithID, mount test.MachineMount, format bool, vm *pkg.VM) error {
...
func (p *Primitives) mountQsfs(wl *gridtypes.WorkloadWithID, mount test.MachineMount, vm *pkg.VM) error {

	var info test.QuatumSafeFSResult
...
func (p *Primitives) virtualMachineProvisionImpl(ctx context.Context, wl *gridtypes.WorkloadWithID) (result test.ZMachineResult, err error) {
	var (
		storage = stubs.NewStorageModuleStub(p.zbus)
	...
	if imageInfo.Container {
		...
		if err != nil {
			return test.ZMachineResult{}, errors.Wrap(err, "failed to create vm rootfs")
		}
		...
		if err != nil {
			return test.ZMachineResult{}, errors.Wrap(err, "failed to get cloud-container flist hash")
		}
