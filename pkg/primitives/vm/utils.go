...
func (p *Manager) newMyceliumNetworkInterface(ctx context.Context, dl gridtypes.Deployment, wl *gridtypes.WorkloadWithID, config *test.MyceliumIP) (pkg.VMIface, error) {
	network := stubs.NewNetworkerStub(p.zbus)
	netID := test.NetworkID(dl.TwinID, config.Network)
...
func (p *Manager) newPrivNetworkInterface(ctx context.Context, dl gridtypes.Deployment, wl *gridtypes.WorkloadWithID, inf test.MachineInterface) (pkg.VMIface, error) {
	network := stubs.NewNetworkerStub(p.zbus)
	netID := test.NetworkID(dl.TwinID, inf.Network)
...
func fListStartup(data *test.ZMachine, path string) error {
	f, err := os.Open(path)
	if os.IsNotExist(err) {
		return nil
	} else if err != nil {
