...
func (p *Manager) getAssignedPublicIP(ctx context.Context, wl *gridtypes.WorkloadWithID) (ip gridtypes.IPNet, gw net.IP, err error) {
	...
	ipWorkloads := deployment.ByType(test.PublicIPv4Type, test.PublicIPType)
	reservedCount := 0
...
func (p *Manager) getPublicIPData(ctx context.Context, wl *gridtypes.WorkloadWithID) (result test.PublicIP, err error) {
	switch wl.Type {
	case test.PublicIPv4Type:
		// backword compatibility with older ipv4 type
		result.V4 = true
	case test.PublicIPType:
		err = json.Unmarshal(wl.Data, &result)
	default:
		return result, fmt.Errorf("invalid workload type expecting (%s or %s) got '%s'", test.PublicIPv4Type, test.PublicIPType, wl.Type)
	}

	return
}

func (p *Manager) publicIPProvisionImpl(ctx context.Context, wl *gridtypes.WorkloadWithID) (result test.PublicIPResult, err error) {
	config, err := p.getPublicIPData(ctx, wl)
	if err != nil {
		return test.PublicIPResult{}, err
	}
	...
	if config.V6 {
		...
		if err != nil {
			return test.PublicIPResult{}, errors.Wrap(err, "could not compute ipv6 valu")
		}
	}
...
func GetPubIPConfig(wl *gridtypes.WorkloadWithID) (result test.PublicIPResult, err error) {
	if wl.Type != test.PublicIPv4Type && wl.Type != test.PublicIPType {
