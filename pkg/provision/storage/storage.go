...
var (
	...
	typeIDfn = map[gridtypes.WorkloadType]func(*gridtypes.Workload) (string, error){
		test.NetworkType: networkTypeID,
	}
)
...
func networkTypeID(w *gridtypes.Workload) (string, error) {
	...
	return string(test.NetworkID(w.User.String(), name.Name)), nil
}
...
func (s *Fs) GetNetwork(id test.NetID) (gridtypes.Workload, error) {
	path := filepath.Join(s.root, pathByType, test.NetworkType.String(), id.String())
	return s.get(path)
}
