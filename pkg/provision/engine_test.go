...
func TestGetMountSize(t *testing.T) {
	...
	t.Run("different data type", func(t *testing.T) {
		wl := gridtypes.WorkloadWithID{
			Workload: &gridtypes.Workload{Type: test.ZDBType, Data: json.RawMessage(`{"size": 10}`)},
		}
		_, err := getMountSize(wl.Workload)
		assert.Error(t, err)
	})
	t.Run("valid data", func(t *testing.T) {
		wl := gridtypes.WorkloadWithID{
			Workload: &gridtypes.Workload{Type: test.ZMountType, Data: json.RawMessage(`{"size": 10}`)},
		}
	...
	t.Run("volumes", func(t *testing.T) {
		wl := gridtypes.WorkloadWithID{
			Workload: &gridtypes.Workload{Type: test.VolumeType, Data: json.RawMessage(`{"size": 10}`)},
		}
		size, err := getMountSize(wl.Workload)
		assert.NoError(t, err)
		assert.Equal(t, size, gridtypes.Unit(10))
