...
It checks the hub for new releases of test packages. Also check version and `safe_to_upgrade` flag from the chain.

If `safe_to_upgrade` is set to `false` and a new release is available then it will only update the [configured farms](https://github.com/threefoldtech/test-config/blob/main/config.schema.json#L49)
