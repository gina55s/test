...
import (
	...
	"github.com/threefoldtech/test/pkg/upgrade/hub"
...
func getRolloutConfig(env environment.Environment) (ChainVersion, []uint32, error) {
	...
	if err != nil {
		return ChainVersion{}, nil, errors.Wrap(err, "failed to get test version from chain")
	}
...
func (u *Upgrader) Run(ctx context.Context) error {
	...
	if method == BootMethodOther {
		...
		// binaries required by the system except for the test
		// binaries
...
// and only update test package if u.noZosUpgrade is set to false
func (u *Upgrader) updateTo(link hub.TagLink, current *hub.TagLink) error {
	...
	for _, pkg := range packages {
		...
		if pkg.Name == ZosPackage {
			// this is the last to do to make sure all dependencies are installed before updating test
	...
	// probably check flag for test installation
