...
import (
	...
	"github.com/threefoldtech/test/pkg/network/iperf"
...
const (
	toZosVeth                   = "totest" // veth pair from br-pub to test
...
// or over test bridge via a veth pair.
...
func GetCurrentPublicExitLink() (netlink.Link, error) {
	...
	// physical nic or to test via an Veth pair then
	...
	return getUplink(
		...
		// a veth pair to another bridge (test always)
		bootstrap.VEthFilter)
}

// GetPrivateExitLink returns the physical link test is wired to
func GetPrivateExitLink() (netlink.Link, error) {
	...
	// test bridge can only be connected to the outside world
	// over a physical nic.
...
// this upstream device can either be a physical free device, or test bridge.
// the method is idempotent.
func SetPublicExitLink(link netlink.Link) error {
	// we can only attach to either physical nic, or test bridge
	log.Debug().
		Str("type", link.Type()).
	...
	if current != nil {
		...
		if veth, _ := bootstrap.VEthFilter(current); veth {
			// br pub is already connected to test
			if link.Attrs().Name == "test" {
				return nil
			}

			// otherwise we remove this link
			if err := netlink.LinkDel(current); err != nil {
				return errors.Wrap(err, "failed to unhook public bridge from test bridge")
			}
...
//
// if no nic is found test is selected.
...
func getPublicUpstream() (netlink.Link, error) {
	...
	// otherwise, it must be the veth to test bridge!
	// so we do
