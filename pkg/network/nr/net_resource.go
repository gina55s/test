...
import (
	...
	"github.com/threefoldtech/test/pkg/network/nft"
...
func (m *MyceliumInspection) IP(seed test.Bytes) (ip net.IPNet, gw net.IPNet, err error) {
...
func (nr *NetResource) MyceliumIP(seed test.Bytes) (ip net.IPNet, gw net.IPNet, err error) {
	if len(seed) != test.MyceliumIPSeedLen {
...
func (nr *NetResource) SetMycelium() (err error) {
	...
	args = AppendFunc(args, config.Peers, func(mp test.MyceliumPeer) string {
		return string(mp)
	})
