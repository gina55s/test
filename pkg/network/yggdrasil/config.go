...
import (
	...
	testCrypto "github.com/threefoldtech/test/pkg/crypto"
...
func GenerateConfig(privateKey ed25519.PrivateKey) NodeConfig {
	...
	if privateKey != nil {
		...
		encryptionPrivateKey := testCrypto.PrivateKeyToCurve25519(privateKey)
		cfg.EncryptionPrivateKey = hex.EncodeToString(encryptionPrivateKey[:])

		encryptionPublicKey := testCrypto.PublicKeyToCurve25519(signingPublicKey)
