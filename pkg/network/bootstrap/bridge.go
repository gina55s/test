...
func DefaultBridgeValid() error {
	...
	// check there is a default route for ipv6 on the test bridge
	...
	if !hasGW {
		...
		if _, err := sysctl.Sysctl("net.ipv6.conf.all.accept_ra", "0"); err != nil {
			log.Fatal().Err(err).Msgf("failed to disable accept_ra test")
		}

		if _, err := sysctl.Sysctl("net.ipv6.conf.all.accept_ra", "1"); err != nil {
			log.Fatal().Err(err).Msgf("failed to enable accept_ra test")
		}
	}

	// test doesn't not need to forward anything, disable forwarding on it
	if _, err := sysctl.Sysctl("net.ipv6.conf.all.forwarding", "0"); err != nil {
		log.Fatal().Err(err).Msgf("failed to disable ipv6 forwarding on bridge test")
	}
