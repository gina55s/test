...
func (d *DualStack) Create(ctx context.Context) error {
	...
	if master == "" {
		...
		// test bridge will not be selected as it should have ipv6 disabled already
		// (or its SLAAC address will be moved to test). It might be that this happens
		// too soon though for all ifaces to properly have configured their SLAAC
		// address.
		// TODO: fix this properly: https://github.com/threefoldtech/test/issues/1159
	...
	// master will actually be `test`. In that case, we can't plug the physical
	// iface, but need to create a veth pair between br-pub and test.
	if !namespace.Exists(NetNSNDMZ) {
		...
		// if the physLink is a bridge (test), create a veth pair. else plug
