...
import (
	...
	"github.com/threefoldtech/test/pkg/network/mycelium"
	...
	"github.com/threefoldtech/test/pkg/network/wireguard"
	"github.com/threefoldtech/test/pkg/network/yggdrasil"
	...
	"github.com/threefoldtech/test/pkg/set"
...
func (n *networker) SetupMyceliumTap(name string, netID test.NetID, config test.MyceliumIP) (tap pkg.PlanetaryTap, err error) {
...
func (n *networker) rmNetwork(wl gridtypes.WorkloadID) error {
	netID, err := test.NetworkIDFromWorkloadID(wl)
	if err != nil {
		return err
	}
...
func (n *networker) GetPublicExitDevice() (pkg.ExitDevice, error) {
	...
	// if exit is over veth then we going over test bridge
...
func (n *networker) networkOf(id test.NetID) (nr pkg.Network, err error) {
...
func (n *networker) Metrics() (pkg.NetResourceMetrics, error) {
	...
	for _, link := range links {
		...
		nsName := n.Namespace(test.NetID(filepath.Base(sym)))
