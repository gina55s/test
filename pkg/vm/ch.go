...
func (m *Machine) Run(ctx context.Context, socket, logs string) (pkg.MachineInfo, error) {
	...
	if err := m.buildZosRC(); err != nil {
		return pkg.MachineInfo{}, errors.Wrap(err, "couldn't build testrc")
	}
	if m.Boot.Initrd != "" {
...
func (m *Machine) buildZosRC() error {
	...
	file, err := os.OpenFile(
		filepath.Join(root, ".testrc"),
		os.O_APPEND|os.O_CREATE|os.O_WRONLY,
		0644,
	)
	if err != nil {
		return errors.Wrap(err, "failed to open testrc file")
	}

	defer file.Close()
	if err := m.appendEnv(file); err != nil {
		return errors.Wrap(err, "couldn't append environment variables to testrc")
	}
	if err := m.appendEntrypoint(file); err != nil {
		return errors.Wrap(err, "couldn't append entrypoint data to testrc")
	}
	return nil
}
