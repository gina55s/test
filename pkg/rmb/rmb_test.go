...
func TestRouter1(t *testing.T) {
	...
	subRouter := router.Subroute("msgbus").Subroute("test").Subroute("statistics")
	subRouter.WithHandler("get", func(ctx context.Context, payload []byte) (interface{}, error) {
		return nil, nil
	})

	router.Subroute("msgbus").Subroute("test").Subroute("statistics")

	l := make([]string, 0)
	router.getTopics("", &l)

	require.Len(l, 1)
	require.Equal([]string{"msgbus.test.statistics.get"}, l)

	_, err := router.call(context.Background(), "msgbus.test.statistics.get", nil)
	require.NoError(err)
}
