...
func (g *Gedis) Reserve(r *provision.Reservation, nodeID pkg.Identifier) (string, error) {
	...
	result, err := Bytes(g.Send("tfgrid.directory.workload_manager", "reservation_register", Args{
		"reservation": res,
	}))
...
func (g *Gedis) Get(id string) (*provision.Reservation, error) {
	result, err := Bytes(g.Send("tfgrid.directory.workload_manager", "workload_get", Args{
		"gwid": id,
	}))
...
func (g *Gedis) Poll(nodeID pkg.Identifier, from uint64) ([]*provision.Reservation, error) {
	...
	result, err := Bytes(g.Send("tfgrid.directory.workload_manager", "workloads_list", Args{
...
func (g *Gedis) Feedback(id string, r *provision.Result) error {
	...
	_, err := g.Send("tfgrid.directory.workload_manager", "set_workload_result", Args{
...
func reservationFromSchema(w types.TfgridReservationWorkload1) (*provision.Reservation, error) {
	...
	// convert the workload description from jsx schema to test types
