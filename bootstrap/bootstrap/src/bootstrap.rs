...
fn boostrap_test(mode: RunMode) -> Result<()> {
    let flist = match mode {
        RunMode::Prod => "test:production:latest.flist",
        RunMode::Dev => "test:development:latest.flist",
        RunMode::Test => "test:testing:latest.flist",
    };
