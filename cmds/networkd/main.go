...
func bootstrap() error {
	f := func() error {
		...
		err := zinit.AddService("dhcp_test", zinit.InitService{
			Exec:    fmt.Sprintf("/sbin/udhcpc -v -f -i %s -s /usr/share/udhcp/simple.script", network.DefaultBridge),
			Oneshot: false,
			After:   []string{},
		})
		if err != nil {
			log.Error().Err(err).Msg("fail to create dhcp_test zinit service")
			return err
		}

		if err := z.Monitor("dhcp_test"); err != nil {
			log.Error().Err(err).Msg("fail to start monitoring dhcp_test zinit service")
			return err
		}
		return nil
	}
