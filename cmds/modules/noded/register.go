...
func registration(ctx context.Context, cl zbus.Client, cap gridtypes.Capacity) (nodeID, twinID uint32, err error) {
	...
	if err != nil {
		return 0, 0, errors.Wrap(err, "failed to get runtime environment for test")
	}
...
func registerNode(
	...
	testIps, testMac, err := netMgr.Addrs(ctx, "test", "")
	if err != nil {
		return 0, 0, errors.Wrap(err, "failed to get test bridge information")
	}
...
func uptime(ctx context.Context, cl zbus.Client) error {
	...
	env, err := environment.Get()
	if err != nil {
		return errors.Wrap(err, "failed to get runtime environment for test")
	}
