...
func netResourceMigration(active []gridtypes.Deployment) error {
	/*
		...
		hence the test.NetworkID function which takes into account all required inputs to make a unique network id.
	...
	*/
	...
	for _, dl := range active {
		for _, wl := range dl.Workloads {
			...
			netId := test.NetworkID(dl.TwinID, wl.Name)
