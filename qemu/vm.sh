...
Usage: vm -n $name [ -r ] [ -d ]
   ...
   -b: bridge for network (default test)
   -g: open GUI
...
cmdline="console=ttyS1,115200n8 test-debug test-debug-vm $kernelargs"
basepath=$(dirname $0)
vmdir="$basepath/$name"
...
qemu-system-x86_64 -kernel $image \
    ...
    -smp $smp \
    -uuid $uuid \
    -netdev bridge,id=test0,br=${bridge} -device virtio-net-pci,netdev=test0,mac="${basemac}1" \
