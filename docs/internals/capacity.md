...
- The system binaries, those what include all test to join the grid and operate as expected
...
This is why the system on first start allocates and reserve a part of the available SSD storage and is called `test-cache`. Initially is `5G` (was 100G in older version) but because the `dynamic` nature of the cache we can't fix it at `5G`
