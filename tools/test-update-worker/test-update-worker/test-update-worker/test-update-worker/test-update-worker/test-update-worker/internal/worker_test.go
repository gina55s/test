...
func TestWorker(t *testing.T) {
	...
	t.Run("test_no_src_qa", func(t *testing.T) {
		...
		if err == nil {
			t.Errorf("update test should fail")
		}
	})

	t.Run("test_no_src_test", func(t *testing.T) {
		_, err := os.Create(src + "/test:v3.4.0-qa1.flist")
		if err != nil {
			t.Error(err)
		}
		...
		if err == nil {
			t.Errorf("update test should fail for test, %v", err)
		}
	})

	t.Run("test_no_src_main", func(t *testing.T) {
		_, err = os.Create(src + "/test:v3.1.1-rc2.flist")
		if err != nil {
			t.Error(err)
		}
		...
		if err == nil {
			t.Errorf("update test should fail for main, %v", err)
		}
	})
