...
func NewWorker(src string, dst string, params Params) (*Worker, error) {
	...
	// - then it will crease dst/test:testing-3:latest.flist that points to dst/test:<v>.flist
...
// updateZosVersion updates the latest test flist for a specific network with the updated test version
func (w *Worker) updateZosVersion(network Network, manager client.Manager) error {
	...
	testCurrent := fmt.Sprintf("%v/.tag-%v", w.src, currentZosVersion)
...
func (w *Worker) UpdateWithInterval(ctx context.Context) {
	...
	for {
		for network, manager := range w.substrate {
			log.Debug().Msgf("updating test version for %v", network)
			...
			if err != nil {
				log.Error().Err(err).Msg("update test failed with error")
			}
		}
